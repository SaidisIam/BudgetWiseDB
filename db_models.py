class StatisticDB():
    ...
    def get_statistic(self, fullname):
        self.cur.execute('select u.fullname,sum(i.amount) as total_income, sum(e.amount) as total_expences, s.net_saving, s.year, s.month from statistics as s join users as u on s.user_id = u.id join income as i on i.user_id = u.id join expences as e on e.user_id = u.id  where fullname = %s group by u.fullname, s.net_saving, s.year, s.month', (fullname,))
        user = self.cur.fetchone()
        return user
    
    def add_income(self, user_id, amount, source, event_date):
        try:
            self.cur.execute("insert into books(title, price) values (%s, %s, %s, %s)", (user_id, amount, source, event_date))
            return True
        except:
            return False
...
class UsersDB():
    ...
    def add_users(self, fullname, username, email, password, balance):
        try:
            self.cur.execute("insert into income(title, price) values (%s, %s, %s, %s, %s)", (fullname, username, email, password, balance))
            return True
        except:
            return False